...
public class User {
    ...
    @Pattern(regexp = "^(?=.*?[A-Z])(?=.*?[a-z])(?=.*2[0-9])(?=.*?[#?!@$%^&*-]).{8}$")/*At least one upper case (?=.*?[A-Z]), At least one lower case (?=.*?[a-z]), At least one digit (?=.*?[0-9]), At least one special character (?=.*?[#?!@$%^&*-]), Max eight in length .{8}*/
